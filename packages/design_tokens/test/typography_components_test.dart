...
void main() {
  ...
  test('ten-foot controls preserve larger accessibility targets', () {
    final phone = FourBaComponents.button(
        kind: FourBaButtonKind.primary, isTenFoot: false);
    final tv = FourBaComponents.button(
        kind: FourBaButtonKind.primary, isTenFoot: true);
    ...
    expect(FourBaComponents.playerControlTarget(isTenFoot: true),
  ...
  test('navigation changes by layout capability including dedicated TV sidebar',
      () {
    expect(FourBaComponents.navigation(FourBaLayoutClass.compact),
        FourBaNavigationKind.bottomBar);
    expect(FourBaComponents.navigation(FourBaLayoutClass.medium),
        FourBaNavigationKind.rail);
    expect(FourBaComponents.navigation(FourBaLayoutClass.expanded),
        FourBaNavigationKind.sidebar);
    expect(FourBaComponents.navigation(FourBaLayoutClass.tv),
        FourBaNavigationKind.tvSidebar);
  });

  test('player semantic controls include recovery without provider identity',
      () {
    expect(FourBaPlayerControl.values,
        contains(FourBaPlayerControl.sourceRecovery));
