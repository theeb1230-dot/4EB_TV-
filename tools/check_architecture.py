...
# Direct package dependencies permitted by MASTER_ARCHITECTURE.
...
def source_imports(package_dir: Path) -> set[str]:
    found: set[str] = set()
    for dart_file in package_dir.rglob("*.dart"):
        text = dart_file.read_text(encoding="utf-8")
        found.update(name for name in PACKAGE_IMPORT.findall(text) if name in ALLOWED)
    return found


def main() -> int:
    errors: list[str] = []
    discovered = {p.name for p in PACKAGES.iterdir() if p.is_dir() and (p / "pubspec.yaml").exists()}
    ...
    for name in sorted(discovered & ALLOWED.keys()):
        ...
        imports = source_imports(package_dir)
