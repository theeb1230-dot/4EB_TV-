...
- **ProxyFill:** Phase-1 audit is complete as an evidence-backed exclusion. The authoritative archive exactly matches six decisive blobs from pinned public commit `1a49d164d10aa24ae5ef57b6a275758500310bae`, including README, project manifests, Program.cs, `global.json`, and the service carrying the hard-coded credential. The bundle contains no LICENSE/NOTICE-named file, so direct reuse fails closed. The authoritative manifest also contains the unsafe `wwwroot/credentials.json` output-copy directive, while that credential file path is absent from the archive. Secret values were never recorded. A bounded text/code scan found no selected ad/analytics/telemetry signatures. Runtime remains EXCLUDED.
