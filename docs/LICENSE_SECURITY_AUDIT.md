...
- **cinextma:** remains Experience-reference pending authoritative snapshot matching. Public `wisnuwirayuda15/cinextma` master commit `0d939f089b5225d49da0edd684346530c5b9431d` is corroborating evidence only: it has an MIT root license, package lock, Supabase auth/database, TMDB, PWA, Turnstile, Vercel Analytics and Speed Insights. Public environment examples use placeholders for Google auth/SMTP configuration. Supabase/account/cloud and Vercel telemetry are not inherited into 4BA Core. Archive reads timed out in this run, so no public license/dependency claim is promoted to the authoritative bundle.
