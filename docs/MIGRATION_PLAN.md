...
- cinextma: `EXPERIENCE_REFERENCE`; mandatory Supabase/cloud/account coupling and Vercel analytics/telemetry are excluded from Core. Public MIT/package-lock/PWA/TMDB evidence is corroborating only until authoritative archive blobs are matched; no player/provider implementation is admitted.
